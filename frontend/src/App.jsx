...
function App() {
  ...
  const handlePullJobs = async (e) => {
    ...
    try {
      ...
      if (resp.ok) {
        setStatus({ status: 'running', query: query });
        startStreaming();
      } else {
        const errorData = await resp.json();
        alert(errorData.detail || "Failed to trigger job pull.");
      }
    } catch (err) {
  ...
  useEffect(() => {
    ...
    if (status.status === 'running') {
      // If we are running, ensure SSE is connected
      if (!eventSourceRef.current || eventSourceRef.current.readyState === EventSource.CLOSED) {
        startStreaming();
      }
      
      // Poll backend state to detect completion
      interval = setInterval(async () => {
        try {
          ...
          if (resp.ok) {
            ...
            setStatus(data);
            if (data.status === 'idle') {
              // Agent finished! Reload jobs list
  ...
  const filteredJobs = useMemo(() => {
    return jobs.filter(job => {
      ...
      return matchesSearch && matchesC2C && matchesSource && matchesLocation;
    });
  }, [jobs, searchTerm, selectedC2C, selectedSource, selectedLocation]);
  ...
  const stats = useMemo(() => {
    ...
    return { total, confirmedC2C, remote };
  }, [jobs]);
    ...
    <div className="app-container">
      ...
      <div className={`dashboard-grid ${status.status === 'running' || logs.length > 0 ? 'with-console' : ''}`}>
        ...
        <aside className="sidebar-panel">
          ...
          <form onSubmit={handlePullJobs} className="control-group">
            <label className="control-label">Search Target</label>
            <input 
              type="text" 
